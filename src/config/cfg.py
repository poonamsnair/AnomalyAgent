...
def process_general(config: MMConfig) -> MMConfig:
    ...
    config.log_path = os.path.join(config.exp_path, getattr(config, 'log_path', 'anomaly_agent.log'))
    logger.info(f"| Arguments Log file: {config.log_path}")

    if "save_path" in config:
        config.save_path = os.path.join(config.exp_path, getattr(config, 'save_path', 'anomaly_detection.json'))

    return config

def process_mcp(config: MMConfig) -> MMConfig:
    if "mcp_tools_config" in config and config.mcp_tools_config is not None:
        mcp_servers = config.mcp_tools_config.get('mcpServers', {})
...
class Config(MMConfig, metaclass=Singleton):
    ...
    def init_config(self, config_path: str, args: Namespace) -> None:
        ...
        # Process MCP configuration only if it exists
        try:
            mmconfig = process_mcp(mmconfig)
        except AttributeError:
            # If mcp_tools_config doesn't exist, skip MCP processing
            pass
