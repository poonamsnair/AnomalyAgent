...
async def create_agent(config):
    ...
    # Load MCP tools (only if mcp_tools_config exists)
    mcpadapt_tools = []
    if hasattr(config, 'mcp_tools_config') and config.mcp_tools_config is not None:
        try:
        ...
        logger.info("No MCP tools configuration found, skipping MCP tool initialization")
